...
for pkg in ["requests", "pandas", "fastapi", "cachetools", "urllib3", "openpyxl", "numpy", "simplejson", "openpyxl", "httpx", "asyncio"]:
...
async def fetch_batch(client, token: str, skipp: int):
...
def process_sources(sources, raw_rows, raw_logs):
...
async def getdata_async(token: str):
    try:
        ...
        skip_list = list(range(0, 30001, 150))
        semaphore = asyncio.Semaphore(5)  # Giới hạn 5 request cùng lúc

        async with httpx.AsyncClient() as client:

            async def sem_fetch(skipp):
                async with semaphore:
                    return await fetch_batch(client, token, skipp)

            tasks = [sem_fetch(skipp) for skipp in skip_list]

            for future in asyncio.as_completed(tasks):
                sources = await future
                size = len(json.dumps(sources).encode('utf-8'))
                total_bytes += size

                process_sources(sources, raw_rows, raw_logs)

                if len(sources) < 170:
                    break  # Không còn batch nào nữa
    ...
    try:
        if secrets == 'chucm@ym@n8686':
            if cache.get(token, {}).get("stores") is None:
                data_stores = await getdata_async(token)
                async with lock:
